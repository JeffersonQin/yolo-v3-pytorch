...
from utils.data import load_data_coco
...
def load_data() -> torch.Tensor:
	...
	"""
	def internal_method():
		train_iter, _ = load_data_coco(1024, train_shuffle=False, data_augmentation=False, pin_memory=False)
		...
		for i, (_, y) in enumerate(train_iter):
			y = y[0].to(try_gpu())
			y = y.reshape(-1, 85) # Here we hard-encode because we are using COCO dataset
			p = (y[:, 4] >= 1)
			Y = torch.cat((Y, y[:, 2:4][p]), dim=0)
			print(f'Loading Data: {i + 1} / {len(train_iter)}')
		return Y

	return internal_method().to('cpu')
...
if __name__ == '__main__':
	...
	train(data, batch_size=1024, K=9, num_epoch=400)
