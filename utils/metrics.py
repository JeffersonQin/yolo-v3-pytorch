...
class ObjectDetectionMetricsCalculator():
	...
	def add_data(self, pred: list[torch.Tensor], truth: torch.Tensor):
		"""Add data for calculating metrics

		Args:
			pred (list[torch.Tensor]): detection prediction, can be either batch result or single result [(#, S, S, (5+num_classes)*B)] or [(S, S, (5+num_classes)*B)]
